...
查看态会把链接渲染为一个全宽长条形按钮，文本靠左，按钮只使用简单描边轮廓和主题 token。点击有效链接时，插件优先通过基础卡片 `openResource` 链路交给卡片查看器调用 Host `resource.open`，最终由系统默认浏览器打开。编辑态提供两个输入框：
...
- `card_type/theme/locale/anchor_text/url` 正式配置模型
