...
`Chips-MusicPlayer` 是薯片生态的应用插件音乐播放器。它沿用已迁移应用的 AppRuntime / Scene / Environment 分层，负责接收 Host `resource.open` / `file-handler` 路由、恢复播放会话，并提供封面、歌词、队列、进度、循环与保存副本等正式能力。

本项目参考了开源仓库 `OpenSourceProjectHandlingFolder/MusicPlayer-main` 的视觉布局与歌词滚动氛围，但运行时实现完全收口在薯片生态正式架构内：

- 外层仍然是标准 `type: app` 应用插件；
- 系统能力通过 `chips-sdk` 和 Host 注入的正式运行时环境调用；
- 主题、语言、surface、权限和诊断通过 `ChipsEnvironmentProvider` 与 `AppRuntimeProvider` 注入；
- 菜单、工具栏、快捷键和媒体动作通过 Host command registry 统一注册；
...
- 多首音频会归一为播放队列，单曲打开是队列长度为 1 的特例
- 自动发现同目录伴生 `cover / folder / front` 封面与同名或 `lyrics.lrc` 歌词
- 支持 MP3 / FLAC / MP4(M4A/AAC) / Ogg Vorbis / Opus 常见嵌入元数据读取：标题、歌手、专辑、封面、歌词
- 嵌入 TIFF 封面优先在 Host 工作区通过 `resource.convertTiffToPng` 归一为 PNG；播放器不复制宿主级图像转换基础设施
- 左封面右歌词布局、滚动歌词、高斯背景氛围层
- 播放 / 暂停、上一首 / 下一首、前后 10 秒、音量、静音、单曲循环
...
- `src/App.tsx`：轻量导出正式 AppRoot
- `src/app/`：AppRoot、Provider、Runtime、Scene 和 AppShell
- `src/runtime/`：SDK client、launch context、环境 client 与主题快照
- `src/commands/`：Host command registry 元数据、状态和 handler 接线
...
- `src/utils/audio-metadata.ts`：MP3 / FLAC / MP4 / Ogg / Opus 嵌入元数据解析
