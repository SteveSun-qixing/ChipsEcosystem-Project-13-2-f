...
暗夜主题的视觉语言必须通过主题 token 与 CSS 落地，不在业务页面、应用结构或组件实现中散落硬编码样式。
...
- `ref`：深色灰阶、状态色、圆角、基础空间等无语义值；
- `sys`：画布、表面、文字、主色、状态色、图标语义等系统级语义；
...
布局原语和容器组件的边框、分隔线、表单控制边界必须使用 `chips.sys.color.border-subtle / border-strong` 或对应状态色，不直接使用 `chips.sys.color.on-surface`。`on-surface` 只承载文字和主要图标，避免暗夜界面出现发白、发硬的线框。
...
当前 `styles/base.css` 中的 `--chips-base-*` 是暗夜主题内部便捷派生变量，用于组织暗色阴影、表面、焦点、控件高度和状态过渡。

这些变量服务暗夜主题 CSS 实现，不是新的跨生态公共契约。跨主题公共 key 必须以生态共用技术文档和组件库 token / contract 为准。

## 数值与单位

暗夜主题可以在布局密度和间距 token 中使用 `cpx`，由 Host Theme Runtime 在浏览器普通 CSS 环境注入前按正式规则转换。主题文档只描述视觉密度和外观节奏，不把 `cpx` 用来规定应用页面结构。
