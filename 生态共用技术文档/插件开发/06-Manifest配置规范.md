...
布局插件必须在 `manifest.yaml` 中声明 `layout` 字段：
...
- `layoutType`：布局插件公开布局类型标识，必须与箱子 `active_layout_type` 以及 `content.layout_configs` 的键一致；
- `displayName`：布局显示名称，用于治理界面、选择器和查看器展示。

正式约束：

- Host 通过 `plugin.getLayoutPlugin({ layoutType })` 解析布局插件；
- 布局插件入口导出的 `layoutDefinition.layoutType` 必须与 `manifest.layout.layoutType` 一致；
- 顶层 `capabilities` 对布局插件可以保留为检索标签，但不得替代 `layout.layoutType` 成为正式解析依据。
