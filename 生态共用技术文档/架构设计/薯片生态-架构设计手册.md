...
### 0.1 适用范围
- 本手册覆盖薯片生态前端与插件运行时全栈接口边界：PAL、Host 内核、Host 服务域、插件运行时、Bridge 传输层、Host Runtime Client（SDK 仅提供调用封装）、UI Hooks、声明式 UI 层、统一渲染层、组件库、新主题系统、应用页面域。
- 目标对象：架构负责人、Host/SDK/组件库维护者、应用插件负责人、主题包开发者、模板维护者。
- 前端框架 vNext 的公共术语、能力域落点和禁止口径，以 `生态共用技术文档/架构设计/19-前端框架vNext公共契约与术语冻结.md` 为统一入口。

### 0.2 架构归属声明（2026-03-06）
- Host 主责 L1-L9 运行时链路（含 Runtime Client、Declarative UI、Unified Rendering）并随 Host 发布。
- 组件库主责 L10 Component Headless（状态机、结构、a11y）。
- SDK 仅作为开发者工具包提供封装、类型、脚手架和测试辅助，不承载运行时主实现。
- 用户设备仅安装 Host 即可运行；SDK 仅在开发和集成阶段按需使用。
...
- 组件层输出硬编码颜色/阴影/圆角。
- 同一能力存在双动作名并长期并行。
- 除官方编辑引擎正式文档化链路外，应用仓私自复制第二套基础卡片渲染/编辑运行时。
...
- `requestId`
- `caller: { id, type, pluginId?, sessionId?, windowId?, permissions? }`
- `timestamp`
- `deadline`
...
- 内核在注册时生成 `route-manifest`，供 SDK 与测试门禁校验。Host 工作区保留动作名数组 `route-manifest.json`，并输出包含 `action/schemaIn/schemaOut/permission/timeoutMs/idempotent/retries` 的 `route-descriptor-manifest.json`；SDK `src/contracts/route-manifest.json` 使用 descriptor manifest 快照作为正式契约门禁输入。
...
| `resource` | `resolve/open/readMetadata/readBinary/convertTiffToPng` | `resourceId -> uri/meta/blob/open-result/png-file` | `resolve/read*` 幂等 | `resource.read` + `file.read/file.write`（转换动作） | `RESOURCE_NOT_FOUND`, `RESOURCE_TIFF_INVALID_SOURCE`, `RESOURCE_TIFF_CONVERSION_FAILED` |
...
| `box` | `pack/unpack/inspect/validate/readMetadata/openView/listEntries/readEntryDetail/resolveEntryResource/readBoxAsset/prefetchEntries/closeView` | `boxFile / boxDir / sessionId + query -> artifact/meta/session/page/resource` | `inspect/validate/readMetadata/listEntries/readEntryDetail/resolveEntryResource/readBoxAsset/closeView` 幂等 | `box.read/box.write` | `BOX_FORMAT_INVALID`, `BOX_OPEN_FAILED`, `BOX_VIEW_SESSION_NOT_FOUND` |
...
### 5.1 插件类型
- `app`：Host 托管的跨平台 `surface` 应用插件；Desktop 当前由 PAL 映射为窗口
- `card`：基础卡片渲染/编辑插件
- `layout`：箱子布局前端插件，基于 Host 提供的箱子会话与摘要态数据渲染页面
- `module`：Host 托管的无界面能力模块
- `theme`：视觉主题插件
...
### 5.3 隔离策略
- App 级隔离：Host 托管的独立 surface 容器；Desktop 当前由 PAL 映射到窗口，但公共语义仍是 surface。
- Card 渲染隔离：Host 通用链路中 iframe 优先，支持受控模式；官方编辑引擎编辑态采用“单卡 iframe + 本地编辑容器”的正式特例，并由共享文档单独规范。
...
### 6.2 Runtime Client
- 职责：参数规范化、协议映射、错误归一、超时控制、会话动作封装与幂等读动作重试。
...
- 副作用分级：
  - `ui-effect`：滚动、聚焦、过渡。
  - `runtime-effect`：调用 Host Runtime Client（可由 SDK 提供开发封装）。
...
L9 Layout Compute 对 `Stack/Grid/Form/List/Section/ScrollView/Table/Navigation` 输出统一布局语义：

- `layoutConstraints`：包含 `axis/overflow/scrollAxis/containerWidthPx/widthPx/heightPx/min/max/gap/columns/rows/columnWidthPx/itemCount/itemExtentPx` 等结构约束。
- `responsive`：包含 `compact/regular/expanded` 断点、缩放比例和 viewport 尺寸。
- `visibleRange`：`List/Grid/Table` 默认输出可见区范围，用于增量提交。
- `performanceMetrics`：包含 `nodeCount/layoutNodeCount/commitNodeCount/pipelineDurations`，供性能门禁与诊断面板统一消费。

`cpx` 换算以当前节点 `containerWidthPx` 为基准，公式为 `px = cpx * (containerWidthPx / 1024)`。L9 只输出布局和响应式语义，不输出视觉样式。

### 8.7 统一渲染入口契约（公开）

统一渲染能力通过 `card.render` 对外暴露，推荐输入输出结构：
...
- `RENDER_CONTRACT_*`：契约校验失败（含能力不兼容）
- `RENDER_THEME_*`：主题 token 解析失败
- `RENDER_LAYOUT_*`：布局计算失败
- `RENDER_PIPELINE_*`：流水线阶段执行失败

`P0/P1` 诊断必须进入 `qualityGate.blockingDiagnostics` 并使 `qualityGate.passed=false`。`P2/info` 可进入诊断列表，但不得阻断质量门禁。SDK、CLI、脚手架测试、预览工具和设置面板统一消费 L9 输出，不定义平行诊断格式。
...
- 生态独有文档与卡片显示层：EmbeddedDocumentFrame（受控文档 iframe 基组件）、CardCoverFrame（卡片封面iframe）、CompositeCardWindow（复合卡片iframe）。
- 每个组件必须声明：
  - 可组合子部件清单
  - 主题接口点清单
  - 可访问性语义清单
...
- `EmbeddedDocumentFrame` 负责承载正式受控文档 iframe，可用于箱子封面、箱子条目封面与应用内受控 iframe 区域
- `CardCoverFrame` 是卡片封面专用包装组件，通过 `Host card.renderCover -> SDK coverFrame.render` 获取受控 `coverUrl`
- 箱子本体正式封面通过 `Host box.renderCover -> SDK client.box.renderCover -> EmbeddedDocumentFrame` 获取受控 `coverUrl`
- 封面 iframe 内容只由 `.card/cover.html` 决定,不自动叠加主题与多语言壳层
...
### 11.6 主题解析算法（运行时）
1. 加载作用域链主题快照（`global -> app -> box -> composite-card -> base-card -> component`）
...
### 16.3 指标责任
- Host 负责路由性能指标。
- Host（内置 Runtime Client/UI Hooks/Renderer）负责 Bridge 调用与渲染链路指标。
- App 负责页面级交互指标。
...
- SDK 内部（开发者工具包）：`api-wrapper/`, `types/`, `tooling/`, `examples/`, `contracts/`。
